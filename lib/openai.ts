  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions:
        ...
        " GRADE CALIBRATION: student_context.curriculumStandard describes CCSS-aligned expectations for the student's target US grade. Evaluate sentence complexity, vocabulary range, and reasoning depth against that standard. The student_context.levelDescription is a secondary signal — grade standard wins on conflicts. Do NOT cite CCSS codes or grade numbers back to the student in feedbackForChild; keep that field warm and specific in Korean." +
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      instructions:
        ...
        " SCORING RULE: 70 is the MODE for an on-level spoken answer, not a floor. Just-meeting the standard scores 70-72, not 80. Reserve 80+ for takes that CLEARLY EXCEED the standard (reframe, multiple perspectives, explicit limits, fluency beyond what the grade requires). 60- for answers that miss the topic or have many basic errors. Individual rubric metrics should average within ±5 of overall score." +
        " GRADE CALIBRATION: student_context.curriculumStandard describes CCSS-aligned expectations for the student's target US grade. Evaluate fluency, sentence complexity, vocabulary range, and reasoning depth against that standard. Do NOT cite CCSS codes or grade numbers back to the student in any Korean output." +
        " GRADE GAP RULE (strict): If the spoken answer's sentence complexity, vocabulary range, OR reasoning depth is CLEARLY BELOW the curriculumStandard for ANY ONE of those three dimensions, score MUST be ≤60 even when the speech is fluent and addresses the topic. Examples that trigger ≤60: registered as Grade 7+ but the answer reads like Grade 3 (5-7 word sentences, no academic vocabulary, fact + 'because' only); registered as Grade 11+ but no reframe / limits / multi-perspective reasoning appears. Do not soften this out of warmth — the parent needs the signal.",
      input: [
        {
          role: "user",
          content: [
